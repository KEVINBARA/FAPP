...
def customer_add_order(request):
    """
        params:
            ...
            restaurant_id
            address
            order_details(json format), it should be in json format ,example:
    ...
    if request.method == "POST":
        ...
        # Checj whether customer has any order that is not delivered
        if Order.objects.filter(customer = customer).exclude(status = Order.DELIVERED):
            return JsonResponse({"status":"fail","error":"Your last order must be completed"})

        # check s
        if not request.POST["address"]:
            return JsonResponse({"status":"failed","error":" is required"})
...
# POST params : access_token,"latitude,longitutde"
@csrf_exempt
